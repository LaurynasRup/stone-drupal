...
(function (Drupal) {
  Drupal.behaviors.tableGallery = {
    attach: function attach(context, settings) {
      var productTable = document.querySelector(".types-table");
      if (productTable && !productTable.classList.contains("js-table-active")) {
        var imageCells = productTable.querySelectorAll(".type-image-cell span");
        var _iterator = _createForOfIteratorHelper(imageCells),
          _step;
        try {
          for (_iterator.s(); !(_step = _iterator.n()).done;) {
            var cell = _step.value;
            var imageUrl = cell.innerText.trim();
